...
# print(requests.get("https://check.torproject.org", proxies=proxies).text[:500])
...
def spawn_osint_mapper():
    # Disabled by Omega Kernel to prevent terminal loops.
    # The Omega Kernel now absorbs all logs.
    pass
...
@app.route('/api/internal/ingest', methods=['POST'])
def api_internal_ingest():
    try:
        data = request.get_json()
        if not data: return jsonify({"status": "error", "message": "No data"}), 400
        
        # Determine ontology class from source or type
        ont_class = "UNKNOWN"
        val = data.get("address") or data.get("wallet") or "Unknown"
        src = data.get("source") or data.get("attribution") or "INTERNAL_AI_SWARM"
        
        if "aml_score" in data:
            ont_class = "MALWARE_IOC" if data["aml_score"] > 75 else "CRYPTO_WALLET"
        elif "risk_score" in data:
            ont_class = "MALWARE_IOC" if data["risk_score"] > 0.75 else "CRYPTO_WALLET"
            
        record_entity(ont_class, val, f"[{src}] (Score: {data.get('aml_score', data.get('risk_score', 'N/A'))})")
...
class DBHandler:
    ...
    def connect_and_prepare(self):
        ...
        elif self.dbtype == "neo4j":
            if GraphDatabase is None: raise RuntimeError("neo4j not installed")
            uri = os.getenv("NEO4J_URI", "")
            user = os.getenv("NEO4J_USERNAME", "")
            pwd = os.getenv("NEO4J_PASSWORD", "")
            self.client = GraphDatabase.driver(uri, auth=(user, pwd))
            self.client.verify_connectivity()
            log(f"Neo4j connected to {uri}", style="green")
        else:
    ...
    def insert_record(self, record):
        try:
            ...
            if self.dbtype == "mongodb":
                ...
                with self.client.session(database=os.getenv("NEO4J_DATABASE", "neo4j")) as session:
                    session.run("MERGE (p:Page {url: $url}) SET p.title = $title", url=record["web_info"].get("url"), title=record["web_info"].get("title"))
        except Exception as e:
    ...
    def close(self):
        try:
            ...
            elif self.dbtype == "neo4j" and self.client: self.client.close()
        except Exception: pass
