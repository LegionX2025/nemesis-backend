...
async def build_neo4j_graph(limit=1000):
    """
    Sync MongoDB entities into Neo4j and perform AUTOClustering.
    Generates a unique Cluster-ID and entity label if not present.
    """
    import uuid
    logger.info("Syncing MongoDB entities to Neo4j and generating AUTOClusters...")
    ...
    for doc in docs:
        ...
        entity_type = doc.get("entity")
        
        if not cluster or cluster == "Unclustered":
            cluster_id = f"CLUSTER-{str(uuid.uuid4())[:8].upper()}"
            cluster = cluster_id
            entity_type = f"entity-{cluster_id}"
            
            # Save back to MongoDB
            collection_entities.update_one(
                {"_id": doc.get("_id")},
                {"$set": {"cluster": cluster, "entity": entity_type}}
            )
            logger.info(f"[AUTOCluster] Generated {cluster} for {address}")
    ...
    logger.info("Neo4j sync and AUTOCluster complete.")
