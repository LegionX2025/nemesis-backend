...
import json
import os
from app.services.scraper_engine import scraper_instance
from app.services.api_rotator import api_rotator
                ...
                while True:
                    ...
                    async with session.get(base_url, params=params, timeout=15) as r:
                        ...
                        if data.get("status") != "1" and "No transactions found" not in data.get("message", ""):
                            ...
                            # It's fine if no tokentx found, just break
                            break
                        ...
                        if isinstance(txs, list):
                            for tx in txs:
                                all_txs.append({
                                    ...
                                    "methodId": tx.get("methodId", "0x")[:10] if action == "txlist" else "0x",
                                    "contractAddress": str(tx.get("contractAddress", "")).lower()
                                })
