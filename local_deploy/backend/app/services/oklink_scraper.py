...
class OKLinkScraper:
    ...
    async def _save_entity(self, address: str, chain: str, tags: list, clustered: list):
        ...
        query = """
        INSERT INTO entities (id, address, chain, type, labels, last_seen) 
        VALUES (?, ?, ?, 'wallet', ?, CURRENT_TIMESTAMP)
        ON CONFLICT(id) DO UPDATE SET 
            labels=excluded.labels, 
            last_seen=CURRENT_TIMESTAMP
        """
        params = [entity_id, addr_lower, chain_upper, labels_json]
    ...
    async def batch_scrape_addresses(self, unique_addresses: list):
        """Scrapes a list of (address, chain) tuples sequentially using a single browser."""
        results_map = {}
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True)
            context = await browser.new_context()
            ...
            for address, chain in unique_addresses:
                # Basic normalization for chain names expected by oklink
                chain_map = {"ETHEREUM": "eth", "POLYGON": "polygon", "BSC": "bsc", "ARBITRUM": "arb", "TRON": "trx", "BITCOIN": "btc", "OPTIMISM": "op"}
                mapped_chain = chain_map.get(chain.upper(), chain.lower())
                
                data = await self.scrape_address(page, mapped_chain, address)
                if data:
                    results_map[address.lower()] = data['tags']
                    # Save to DB asynchronously but without blocking the batch loop
                    asyncio.create_task(self._save_entity(address, chain, data['tags'], data['clustered']))
