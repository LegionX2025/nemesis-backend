...
from app.services.oklink_scraper import OKLinkScraper
from app.services.godmode_ml import autonomous_classify_node
...
def detect_chain(val: str, override: str = "AUTO") -> str:
    ...
    elif (val.startswith("T") or val.startswith("t")) and len(val) == 34: return "TRON"
    elif val.startswith("0x") and len(val) == 42: return "EVM_AUTO"
    elif (val.startswith("r") or val.startswith("R")) and 25 <= len(val) <= 35: return "RIPPLE"
    elif (val.startswith("G") or val.startswith("g")) and len(val) == 56: return "STELLAR"
    elif 32 <= len(val) <= 44 and not val.startswith("0x") and not val.startswith("bc1") and not (val.startswith("T") and len(val) == 34): return "SOLANA"
    return "INVALID"
...
class TraceEngine:
    ...
    async def process_evm_txs(self, session, addr, txs, depth, carry_val, obf_path, chain, origin_seed):
        for tx in txs:
            ...
            try: ts = datetime.fromtimestamp(int(str(tx.get("timeStamp", 0)), 0)).strftime('%Y-%m-%d %H:%M:%S')
    ...
    async def engine_worker(self, session):
        while True: 
            ...
            try:
                ...
                try:
                    from app.services.godmode_ml import autonomous_classify_node
                    from app.services.swarm_fetcher import swarm_instance
                    ...
                    for key in api_keys:
                        ...
                        try:
                            ...
                            for model_name in model_order:
                                try:
                                    url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name.strip()}:generateContent?key={key}"
                                    async with httpx.AsyncClient() as http_client:
                                        resp = await http_client.post(url, json=prompt_payload)
                                        data = resp.json()
                                    ...
                                    if "candidates" in data and data["candidates"]:
                                        ...
                                        logger.debug(f"    [WARN] Model {model_name} failed: {data}. Trying next model...")
