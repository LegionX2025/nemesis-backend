...
def main():
    ...
    test_script = os.path.join(current_dir, "ops", "test_all.ps1")
    if os.path.exists(test_script):
        # Run tests directly using current Python executable
        run_command(f"{sys.executable} -m pytest backend/tests/", cwd=current_dir, exit_on_error=True, allow_healing=False)
    else:
        print("    -> [SKIP] No test_all.ps1 found.")

    print("\n>>> [2/5] NPM Audits & Updates")
    worker_dir = os.path.join(current_dir, "frontend", "nemesis-global-worker")
