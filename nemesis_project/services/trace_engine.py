...
async def fetch_oklink_label(session, chain: str, address: str) -> str:
    ...
    try:
        ...
        from oklink_scraper import scrape_oklink_tags, scrape_oklink_entity
        ...
        if legacy_result and legacy_result.get("attributionTags"):
            ...
            try:
                await graph_db.tag_wallet(address, primary_tag, cname)
            except Exception as e:
                logger.error(f"Neo4j persistence error: {e}")
            return primary_tag
    except Exception as e:
        logger.warning(f"Playwright scraper failed or skipped: {e}")
