...
@app.get("/recovery_framework")
async def recovery_framework_page(request: Request):
    return templates.TemplateResponse(request=request, name="recovery_framework.html")
...
async def trigger_auto_deploy(current_user: dict = Depends(verify_access_token)):
...
async def api_start_trace(req: TraceRequest, request: Request, current_user: dict = Depends(verify_access_token)):
    try:
...
async def api_trace_autonomous(req: TraceRequest, request: Request, current_user: dict = Depends(verify_access_token)):
    try:
        raw_seeds = req.seeds.replace('"', '').replace("'", "")
        tokens = re.split(r'[\s,]+', raw_seeds)
