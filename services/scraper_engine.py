...
class AutoScraper:
    ...
    async def scrape_oklink(self, address: str, chain: str) -> tuple:
        ...
        try:
            ...
            multi_search_label = None
            multi_search_cluster = None
            ...
            if not any(k in text_blob.lower() for k in ["exchange", "mixer", "hot wallet"]):
                ...
                try:
                    await page.goto(multi_url, wait_until="domcontentloaded", timeout=15000)
                    
                    # Dynamic Loading: Always wait for container
                    await page.wait_for_selector('.home-container', timeout=10000)
                    await page.wait_for_timeout(1000)
                    
                    # Extract entity details using precise Playwright locators
                    multi_results = await page.evaluate('''() => {
                        const items = Array.from(document.querySelectorAll('.home-container a[href*="/address/"], .home-container a[href*="/token/"]'));
                        return items.map(item => {
                            // Use textContent on specific container to bypass ellipsis truncation
                            const labelEl = item.querySelector('.wrapper-qoNkf.oklink-ignore-locale, span[class*="tokenName"]');
                            let exactLabel = labelEl ? labelEl.textContent.trim() : "";
                            
                            const lines = item.innerText.split('\\n').map(s => s.trim()).filter(Boolean);
                            return {
                                label: exactLabel || lines[0] || "",
                                symbol: lines.length > 1 ? lines[1] : "",
                    ...
                    if multi_results and len(multi_results) > 0:
                        # Find the best non-trivial label
                        best_match = multi_results[0]
                        for r in multi_results:
                            if r['label'] and r['label'].lower() != address.lower() and r['label'] != 'Unknown':
                                best_match = r
                                break
                        
                        if best_match['label']:
                            lbl = best_match['label']
                            if best_match['symbol']:
                                lbl += f" {best_match['symbol']}"
                            multi_search_label = lbl.strip()
                            
                            # Auto-label Globalize cluster logic
                            if best_match['type'] == 'Token':
                                multi_search_cluster = "TOKEN"
                            elif "exchange" in lbl.lower():
                                multi_search_cluster = "EXCHANGE"
                            elif "mixer" in lbl.lower():
                                multi_search_cluster = "MIXER"
                            else:
                                multi_search_cluster = "WALLET"
                                
                except Exception as e:
                    logger.debug(f"OKLink multi-search fallback failed: {e}")
            
            label, cluster = self._classify_text(text_blob)
            
            # Apply our precise multi-search label if classification failed or token detected
            if multi_search_label:
                if not label or multi_search_cluster in ["TOKEN", "EXCHANGE", "MIXER"]:
                    label = multi_search_label
                    cluster = multi_search_cluster
    ...
    async def resolve_address(self, address: str, chain: str, trace_id: str = None) -> dict:
        ...
        # Try GBEO v3 NEMESIS CLUSTER INJECTION
        try:
            nemesis_results = await self.run_nemesis_cluster(address)
            if nemesis_results:
                valid_classes = [r.get("classification") for r in nemesis_results if r.get("classification") and r.get("classification") != "Unknown"]
                if valid_classes:
                    label = nemesis_results[0].get("uiLabel", {}).get("mainLabel") or valid_classes[0]
                    cluster = valid_classes[0] # GBEO mapped classification
        except Exception as e:
            logger.error(f"GBEO v3 Override failed: {e}")
    ...
    # ==============================================================
    # NEMESIS GBEO v3 AUTO-RESOLVER & PLAYWRIGHT CLUSTER
    # ==============================================================
    ...
    def _normalize_classification(self, tags: list, label: str) -> str:
        """Normalizes OKLink tags to NEMESIS Ontology."""
        combined = []
        if tags:
            combined.extend([str(t).lower() for t in tags if t])
        if label:
            combined.append(str(label).lower())
            
        for key, val in self.ONTOLOGY_MAP.items():
            for c in combined:
                if key in c:
                    return val
        return 'Unknown'

    async def scan_chain_oklink(self, address: str, chain: str):
        """Intercepts API calls on OKLink to extract hidden cluster IDs."""
        if not self.context:
            return {"chain": chain, "address": address, "clusterData": None, "uiLabel": None, "classification": "Unknown"}
            
        page = await self.context.new_page()
        results = {"chain": chain, "address": address, "clusterData": None, "uiLabel": None}
        ...
        async def handle_response(response):
            if "search/aggregate" in response.url or "/priapi/" in response.url:
                try:
                    ...
                    if data:
                        results["clusterData"] = data.get("data", data)
                except:
                    pass
        ...
        try:
            target_url = f"https://www.oklink.com/{chain}/address/{address}"
            await page.goto(target_url, wait_until="networkidle", timeout=20000)
            
            ui_label = await page.evaluate('''() => {
                const mainLabelEl = document.querySelector('h1, .tokenName-XSePS');
                const mainLabel = mainLabelEl ? mainLabelEl.innerText.trim() : null;
                const tagEls = Array.from(document.querySelectorAll('[class*="tag"], [class*="label"]'));
                const tags = tagEls.map(el => el.innerText.trim()).filter(Boolean);
                return { mainLabel, tags };
            }''')
            
            results["uiLabel"] = ui_label
        except Exception as e:
            logger.error(f"[GBEO v3] Error scanning OKLink chain {chain} for {address}: {e}")
        finally:
            await page.close()

        ui_label_obj = results.get("uiLabel") or {}
        results["classification"] = self._normalize_classification(
            ui_label_obj.get("tags", []), 
            ui_label_obj.get("mainLabel", "")
        )
        return results

    async def run_nemesis_cluster(self, address: str, max_parallel: int = 5):
        """Discovers active chains and runs parallel GBEO scans."""
        if not self.context: return []
        
        discovery_page = await self.context.new_page()
        chains = []
        try:
            await discovery_page.goto(f"https://www.oklink.com/multi-search#key={address}", wait_until="networkidle", timeout=20000)
            # Wait for results container
            try:
                await discovery_page.wait_for_selector('.home-container', timeout=10000)
            except:
                pass # Continue if timeout, might still find links
            
            chains = await discovery_page.evaluate('''() => {
                const links = Array.from(document.querySelectorAll('a[href*="/address/"]'));
                const uniqueChains = new Set(links.map(l => {
                    const parts = new URL(l.href).pathname.split('/');
                    return parts[1] ? parts[1].toLowerCase() : null;
                }).filter(Boolean));
                return Array.from(uniqueChains);
            }''')
        except Exception as e:
            logger.error(f"[GBEO v3] Multi-search discovery failed for {address}: {e}")
        finally:
            await discovery_page.close()

        if not chains:
            logger.warning(f"[NEMESIS] No active chains detected via multi-search for {address}. Defaulting to ETH/BTC.")
            chains = ["eth", "btc"]
            
        logger.info(f"[NEMESIS] Detected {len(chains)} active chains for {address}. Starting parallel scan...")
        
        # We process them sequentially or via asyncio.gather. We'll use gather for speed.
        import asyncio
        tasks = [self.scan_chain_oklink(address, chain) for chain in chains[:max_parallel]]
        ...
        for r in scan_results:
            if isinstance(r, dict):
                ...
                logger.error(f"[GBEO v3] Scan task failed: {r}")
