...
from services.trace_engine import TraceEngine, init_mongodb, mongo_db, get_asset_ticker, detect_chain, EVM_DOMAINS, get_active_traces, get_mongo_status, fetch_saved_traces
from services.auth_engine import authenticate_admin, create_access_token, verify_access_token, ACCESS_TOKEN_EXPIRE_MINUTES
from services.global_data_engine import global_db
...
async def run_boot_diagnostics():
    ...
    try:
        # subprocess.Popen([sys.executable, "auto_backup.py"]) # DISABLED due to disk space issues
        logger.info("    [OK] Auto-backup routine disabled to save space.")
    except Exception as e:
...
async def lifespan(app: FastAPI):
    ...
    async def init_background():
        await init_mongodb()
        await global_db.connect_all()
        from services.scraper_engine import scraper_instance
        await scraper_instance.start()
        
        # Start Godmode Maintenance Task
        from services.maintenance_scheduler import weekly_maintenance_loop
        asyncio.create_task(weekly_maintenance_loop(mongo_db))
        ...
        try:
            ...
            # Open the darknet console in a new, completely detached terminal window
            ...
            logger.info("    [OK] Darknet console spawned via batch file.")
        except Exception as e:
...
@app.get("/report_template.html")
async def report_template_page(request: Request):
    return templates.TemplateResponse(request=request, name="report_template.html")
...
async def nemesis_id_profile(address: str):
    ...
    entity = await mongo_db.entities.find_one({"address": address})
    
    if entity:
        labels = entity.get("labels", [])
        return {
            "address": address,
            "network": entity.get("chain", "Unknown"),
            "entity": labels[0] if labels else "Unknown / Unlabeled",
            "balance": "0.00",
            "first_activity": "N/A",
            "last_activity": str(entity.get("last_seen", "N/A")),
            "total_sent": "0.00",
            "total_received": "0.00",
...
async def nemesis_id_aml(address: str):
    ...
    doc = await mongo_db.entities.find_one({"address": address})
    labels = doc.get("labels", []) if doc else []
    
    risk_score = 0
    risk_level = "Unknown"
    if any("MIXER" in l.upper() for l in labels) or any("HACK" in l.upper() for l in labels):
        risk_score = 100
        risk_level = "Critical"
    elif any("CEX" in l.upper() for l in labels):
        risk_score = 10
        risk_level = "Low"
    ...
    return {
        ...
        "flags": ["High-Risk Entity Detected"] if risk_score > 80 else [],
        "sanctions": []
    }
...
async def nemesis_id_intel(address: str):
    from services.trace_engine import mongo_db
    if mongo_db is None: return {"error": "MongoDB not connected"}
    
    doc = await mongo_db.entities.find_one({"address": address})
    labels = doc.get("labels", []) if doc else []
    
    return {
        "intel_summary": f"Known tags: {', '.join(labels)}" if labels else "No intelligence found.",
        "tags": labels,
        "sources": ["On-Chain Trace", "Scraper Engine"] if labels else []
    }
...
async def nemesis_id_generate_report(req: NemesisIDReportRequest):
    ...
    doc = await mongo_db.entities.find_one({"address": req.address})
    labels = doc.get("labels", []) if doc else []
    
    if not labels:
         return {"markdown": f"# Automated Report for {req.address}\n\nNo structured intelligence available to generate a full report."}
         
    return {
        "markdown": f"# Forensic Report for {req.address}\n\n## Overview\nThis entity has been classified with the following tags: {', '.join(labels)}."
    }
...
async def update_config(config: ConfigModel, token: dict = Depends(verify_access_token)):
    ...
    if config.max_depth: engine.MAX_DEPTH = config.max_depth
    if config.max_hops: engine.MAX_HOPS = config.max_hops
    return {"status": "success"}

@app.get("/api/admin/maintenance_reports")
async def get_maintenance_reports(token: dict = Depends(verify_access_token)):
    if mongo_db is None:
        return {"reports": []}
    
    try:
        reports = await mongo_db.maintenance_reports.find().sort("timestamp", -1).to_list(length=50)
        for r in reports:
            r["_id"] = str(r["_id"])
        ...
        logger.error(f"Failed to fetch maintenance reports: {e}")
        return {"reports": []}

@app.post("/api/admin/maintenance/run")
async def trigger_maintenance_run(token: dict = Depends(verify_access_token)):
    from services.maintenance_scheduler import run_maintenance_task
    import asyncio
    # Fire and forget
    asyncio.create_task(run_maintenance_task(mongo_db))
    return {"status": "Task triggered in background. Check reports shortly."}

@app.post("/api/db/import")
async def api_db_import(data: dict, token: dict = Depends(verify_access_token)):
    res = await global_db.global_import(data)
    return {"status": "success", "results": res}

@app.get("/api/db/search")
async def api_db_search(q: str, token: dict = Depends(verify_access_token)):
    res = await global_db.global_search(q)
    return {"status": "success", "results": res}

@app.get("/api/db/export")
async def api_db_export(token: dict = Depends(verify_access_token)):
    res = await global_db.global_export()
    return {"status": "success", "results": res}
